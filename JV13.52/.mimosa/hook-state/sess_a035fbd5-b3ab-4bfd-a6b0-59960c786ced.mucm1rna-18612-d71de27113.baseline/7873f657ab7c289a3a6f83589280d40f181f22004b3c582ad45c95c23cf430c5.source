...
# -*- coding: utf-8 -*-
"""
File name: 80010001.py
Author: Al Azif
Created: 2021-07-25
Version: 1.0.0
Description:
    Automatically extracts Secure Module keys from a decrypted Secure Kernel
    binary (80010001). It uses the offset of a specific string as a reference
    point to locate the keys. This method has remained valid from firmware
    versions 1.00 through 13.50, though it may change in future updates.
License: GPLv3
...
# Constants
# The anchor string used to locate the key data block.
ANCHOR_STRING: bytes = b"sceSblSkLoadElfReadHeader\0\0\0"
# The length of the anchor string.
ANCHOR_LENGTH: int = len(ANCHOR_STRING)
# The struct format for unpacking keys. This struct appears consistent across known firmware versions.
# Struct uses '>' for big-endian format. 's' denotes bytes (for keys/IVs/RSA components), and 'x' denotes padding bytes to skip.
STRUCT_FORMAT: str = ">16s16s24x256s4x4s256s"

# Type alias for the dictionary structure returned
# Example structure:
# {
#     "SELF": {
#         "AM32": { "5.05_SECURE_MODULE": { "AES_KEY": "...", ... } }
#     }
# }
...
def _get_key_offset(data: Union[bytes, mmap.mmap]) -> Optional[int]:
    ...
    Searches for the anchor to calculate the initial offset

    Args:
        data: The binary data (bytes or mmap object) of the Secure Kernel.

    Returns:
        The calculated integer offset if successful, None otherwise.
    """

    # Basic check: Ensure data is large enough for the anchor itself.
    ...
    # Find the offset of the anchor string.
    offset = data.find(ANCHOR_STRING)
    if offset == -1:
        logging.error(f"Anchor string '{ANCHOR_STRING.decode(errors='ignore')}' not found.")
        return None
    logging.debug(f"Found anchor at offset 0x{offset:X}.")

    # Calculate the offset for key data block.
    offset = offset + ANCHOR_LENGTH
    logging.debug(f"Calculated key offset (immediately after anchor): 0x{offset:X}.")
...
def _process_data(data: Union[bytes, mmap.mmap]) -> Optional[Tuple[float, Tuple]]:
    """
    Processes the Secure Kernel data to extract keys.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Secure Module Key Extractor by Al Azif\nExtracts Secure Module keys from a decrypted Secure Kernel binary (`80010001`).", formatter_class=argparse.RawTextHelpFormatter)
    # Verbosity group
    ...
    parser.add_argument("file", type=pathlib.Path, help="Specify the location of the decrypted Secure Kernel binary")
